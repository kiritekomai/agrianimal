...
public class EntityAIMyFollowOwner extends EntityAIBase {
	private final EntityAgriAnimal tameable;
	...
	public EntityAIMyFollowOwner(EntityAgriAnimal tameableIn, double followSpeedIn, float minDistIn, float maxDistIn) {
		...
		if (!(tameableIn.getNavigator() instanceof PathNavigateGround)
				&& !(tameableIn.getNavigator() instanceof PathNavigateFlying)) {
	...
	public boolean shouldExecute() {
		...
		if (entitylivingbase == null) {
			...
			return false;
		} else if (this.tameable.isHarvesting()) {
			return false;
	 ...
	 */
	public void tick() {
		this.tameable.getLookHelper().setLookPositionWithEntity(this.owner, 10.0F,
				(float) this.tameable.getVerticalFaceSpeed());
		if (!this.tameable.isSitting()) {
			if (!this.tameable.isHarvesting()) {
								...
								for (int l = 0; l <= 4; ++l) {
									for (int i1 = 0; i1 <= 4; ++i1) {
										if ((l < 1 || i1 < 1 || l > 3 || i1 > 3)
											...
											this.tameable.setLocationAndAngles((double) ((float) (i + l) + 0.5F),
